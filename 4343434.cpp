...
bool PreprocessАssistant(const path& in_file, const path& out_file, const vector<path>& include_directories, const path& name_file, const unsigned int number_line) {
    bool found = false;
    ...
    if (!found) {
        cout << "unknown include file "s << name_file.string() << " at file "s << in_file.string() << " at line "s << number_line << endl;
    }
    return found;
}
    ...
    while (getline(reader, line)) {
        ...
        if (regex_match(line, m_file, include_file)) {
            ...
            if (reader_file.is_open()) {
                ...
                bool found = PreprocessАssistant(in_file, out_file, include_directories, name_file, number_line);
